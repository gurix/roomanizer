...
RSpec.describe Booking do
  ...
  it 'validates availability' do
    room = create :room, bookings:[
      Booking.new(start_at: Time.now + 1.hour, end_at: Time.now + 3.hour)
    ]
    ...
    expect(invalid_booking.errors.full_messages).to include('Location not available for that time')

    another_invalid_booking = Booking.new(bookable: room, start_at: Time.now + 30.minutes, end_at: Time.now + 2.hours)
    expect(another_invalid_booking).not_to be_valid
    expect(another_invalid_booking.errors.full_messages).to include('Location not available for that time')
  end
end
